...
words <- c('chocolate','weka','fantastic','overlord','artificial','tango')
...
g_m_words <- words[substr(words,1,1) >= 'g' & substr(words,1,1) <= 'm']
...
word_bin <- function(v_words,l1,l2){
  v_words[substr(v_words,1,1) >= l1 & substr(v_words,1,1) <= l2 ]
}
...
word_bin(c('cookie','elephant','quake','zulu'),'e','q')
